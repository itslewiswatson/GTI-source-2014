...
addEventHandler("onClientResourceStart", resourceRoot, function()
	triggerServerEvent("GTIaccounts.showLoginPanelServer", root)
end)
...
function showLoginPanel()
	...
	local gX,gY = 231,76
	logo = guiCreateStaticImage(sX-gX-5, 5, gX, gY, "logo.png", false)
...
function loginPlayer(button, state)
	...
	triggerServerEvent("GTIaccounts.loginPlayer", root, username, password, rememberMe, isEncrypted)
end
...
function renderRegisterDisplays(edit)
	...
	if (edit == register.edit[1]) then
		...
		elseif (#username < 5 or #username > 25) then
			guiSetText(register.label[2], "Username Must be 5-25 Characters Long")
	...
	if (edit == register.edit[3]) then
		...
		elseif (#password < 5 or #password > 25) then
			guiSetText(register.label[8], "Password Must be 5-25 Characters Long")
...
function checkUsernameValidity()
	local username = guiGetText(register.edit[1])
	if ((#username == 0) or (#username < 5 or #username > 25) or (string.gsub(username, "%W", "") ~= username)) then return end
	
	triggerServerEvent("GTIaccounts.checkUsernameValidity", root, username)
end
...
function registerPlayer(button, state)
	...
	-- Username
	local username = guiGetText(register.edit[1])
	if ((#username == 0) or (#username < 5 or #username > 25) or (string.gsub(username, "%W", "") ~= username)) then
	...
	-- Password
	local password = guiGetText(register.edit[3])
	if ((#password == 0) or (#password < 5 or #password > 25)) then
	...
	triggerServerEvent("GTIaccounts.registerPlayer", root, username, email, password)
end
...
function leaveServer(button, state)
	...
	triggerServerEvent("GTIaccounts.leaveServer", root)
end
