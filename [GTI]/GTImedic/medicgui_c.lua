...
addEventHandler( "onClientResourceStart", resourceRoot,
	function()
		...
		for i, v in ipairs (labelChanges) do
			guiLabelSetColor( v[1], 0, 255, 255)
		end
	end
)
...
addEventHandler( "onClientMouseEnter", root,
	function()
		if source == GTImedic.label[1] then
			guiLabelSetColor( source, 0, 255, 255)
...
function markPlayer( theType, player)
	...
	if theType == "player" then
		...
		if name then
			if marks[player] == nil then
				outputChatBox( "You have marked "..name, 0, 255, 255)
				local blip = createBlipAttachedTo( player, 41, 2, 255, 0, 0, 255, 0, 450)
				marks[player] = blip
			else
				outputChatBox( "You have unmarked "..name, 0, 255, 255)
	...
	elseif theType == "all" then
		for i, players in ipairs (getElementsByType( "player")) do
			...
			if blips == nil then
				--outputChatBox( "You have marked all players", 0, 255, 255)
				if players ~= localPlayer then
					local blip = createBlipAttachedTo( players, 41, 2, 255, 0, 0, 255, 0, 450)
					marks[players] = blip
				end
			else
				--outputChatBox( "You have unmarked all players", 0, 255, 255)
