...
function isMailCarrier(player)
	...
	if (exports.GTIjobs:getPlayerJob(true) == "Mail Carrier") then return true end
	return false
end
...
function createDestinations(player, mailvan, isLoaded)
	if (not isMailCarrier(player) or not isElement(mailvan)) then return end
	if (packages <= 0) then 
		returnToWarehouse(localPlayer)
	return end
...
function deliverPackage(player, dim)
	...
	setTimer(function()
		...
		setTimer(createDestinations, 5000, 1, localPlayer, mailvan)
	end, 1000, 1)
end
...
function returnToWarehouse(player)
	...
	wareMarker = createMarker(x, y, z, "cylinder", 1, 255, 200, 0, 150)
...
function onWarehouseReturn(player, dim)
	...
	setTimer(collectPackages, 4000, 1, localPlayer, 0, vehicle)
end
